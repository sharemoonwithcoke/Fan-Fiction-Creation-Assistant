...
/**
 * Platform-agnostic visual story interpreter shared between web and MP.
 * Identical logic to packages/web/src/hooks/useVisualStoryPlayer.ts —
 * both live in their own package because Taro and React DOM have separate module graphs.
 */
export function useVisualStoryPlayer(script: StoryScript, assets: VisualAssetMap) {
  ...
  const [display, setDisplay] = useState<PlayerDisplay>({
    displayText: '', speaker: '', choices: [], sceneState: EMPTY_SCENE, isEnded: false, isStarted: false,
  })
  ...
  function runUntilPause(sectionId: string, startIdx: number, scene: VisualSceneState) {
    ...
    while (i < section.blocks.length) {
      ...
      switch (block.type) {
        case 'narration':
          sceneRef.current = s; blockIdxRef.current = i
        ...
        case 'dialogue': {
          ...
          sceneRef.current = s; blockIdxRef.current = i
          setDisplay({ displayText: block.text, speaker: char?.name ?? '', choices: [], sceneState: s, isEnded: false, isStarted: true })
          return
        }
        case 'scene':
  ...
  const start = useCallback(() => {
    sectionIdRef.current = script.startSectionId; blockIdxRef.current = 0; sceneRef.current = EMPTY_SCENE
    runUntilPause(script.startSectionId, 0, EMPTY_SCENE)
  }, [script, assets])
  ...
  const choose = useCallback((targetSectionId: string) => {
    sectionIdRef.current = targetSectionId; blockIdxRef.current = 0
